...
- 网友:单次mini梯度肯定是下降的，但因为只拟合了一个mini，所以准确性就没有总体那么高，不过胜在速度快.
...
- 网友：“因为原来是用同样样本集迭代训练，现在相当于一个样本集合训练了之后拿下一个新样本训练，所以误差可能会增大 

你需要决定的变量之一是**mini-batch**的大小，$m$就是训练集的大小，极端情况下，如果**mini-batch的大小等于$m$**，其实就是**batch**梯度下降法，在这种极端情况下，你就有了**mini-batch**  $X^{\{1\}}$和$Y^{\{1\}}$，并且该**mini-batch**等于整个训练集，所以把**mini-batch**大小设为$m$可以得到**batch**梯度下降法。

![](../images/2181fbdc47d4d28840c3da00295548ea.png)

- 我:上图介绍了minibatch大小的极端情况.

另一个极端情况，**假设mini-batch大小为1**，就有了新的算法，叫做随机梯度下降法，每个样本都是独立的**mini-batch**，当你看第一个**mini-batch**，也就是$X^{\{1\}}$和$Y^{\{1\}}$，如果**mini-batch**大小为1，它就是你的第一个训练样本，这就是你的第一个训练样本。接着再看第二个**mini-batch**，也就是第二个训练样本，采取梯度下降步骤，然后是第三个训练样本，以此类推，一次只处理一个。
...
相反，在随机梯度下降法中，从某一点开始，我们重新选取一个起始点，每次迭代，你只对一个样本进行梯度下降，大部分时候你向着全局最小值靠近，有时候你会远离最小值，因为那个样本恰好给你指的方向不对，因此随机梯度下降法是有很多噪声的，平均来看，它最终会靠近最小值，不过有时候也会方向错误，因为**随机梯度下降法永远不会收敛，而是会一直在最小值附近波动**，但它并不会在达到最小值并停留在此。

**实际上你选择的mini-batch大小在二者之间**，大小在1和$m$之间，而1太小了，$m$太大了，原因在于如果使用**batch**梯度下降法，**mini-batch**的大小为$m$，每个迭代需要处理大量训练样本，该算法的主要弊端在于特别是在训练样本数量巨大的时候，**单次迭代耗时太长**。如果训练样本不大，**batch**梯度下降法运行地很好。
...
- 我:蓝色代表batch梯度下降算法;紫色代表随机梯度下降算法的梯度下降路线;绿色的代表minibatch
...
我：这个概念可以先参考阅读下[知乎文章](通俗理解指数加权平均 - 忆臻的文章 - 知乎 https://zhuanlan.zhihu.com/p/29895933)
...
- 网友问：那这个公式为什么要用v和theta两个参数来表示当天温度?
  - 网友答：意思是离当前时间越远的样本对这个平均数的影响越小
  - 网友·：因为theta表示的是你真实数据集中的数据，但是V表示的是你加权计算过的数据；每天只算0.1的权重；Vt表示预测的到今天为止的10天内的平均气温
...
- 我：根据β，可以知道估计的是几天内的平均气温。β==0.9的时候平均了10天；β==0.98的时候平均了20天。
...
- 网友：这和我们人类记忆类似，昨天的事情你记得清楚，上周的就不见得了
...
![image-20220910163225797](lesson2-week2.assets/image-20220910163225797.png)

- 网友：为后面的课程做铺垫， 比如RMSProp优化， 不然你理解不了
- 我：Vt是慢慢变得的，因为t每增大一点，Vt就会增大加一个值；从恩达的图像上看，斜率越来越大，说明增加的量越来越多。
- 高赞网友：这个例子就是告诉我们前面几天的温度对第100天的温度影响多大，权重越大影响越大
...
- 网友：就是一种计算平均数的方法，叫做指数加权平均，比传统的加一起再除以个数要有一些优势。
- 网友：指数加权平均本质就是以指数式递减加权的移动平均。各数值的加权而随时间而指数式递减，越近期的数据加权越重，但较旧的数据也给予一定的加权。
- 网友问：为什么不保存前面所有温度的总和，然后算平均呢，这样也不占内存啊
  - 网友答：你只记住总和，你怎么更新呢？你饶了一圈发现还是只有指数加权平均能这样。
...
- 我和网友问：为什么不让V_0等于theta_1？
  - 网友：带入v0 = 40 你自己手动计算一下，会发现同样v2很小。不能单纯就看v1
...
![image-20220911111005115](lesson2-week2.assets/image-20220911111005115.png)

- 我：红色箭头可以看出，指数加权平均 结合 梯度下降后，梯度的摆动变缓慢了，这样就可以使用更大的学习率而不用担心摆动过大。
...
- 网友：就是每一次迭代，都要考虑之前的导数，就像惯性，动量就是一种惯性
...
- 网友：sw越大w的变化率越大，所以每走一步要减小变化，这样走出来的曲线就变得平滑一点.
- 我：盲猜，这里b的方向和w垂直，也有一种偏置的感觉，对应了b是偏置的概念。
- 网友：一点理解，不一定对：因为对纵轴求导和对横轴求导的小大不一样，S值就是为了使大的更大，小的更小，这样在作为分母时，就能在梯度上呈现出相反的情况，减小震荡的同时加速梯度下降。（除以一个大的，步子变小，除以一个小的，步子变大）
- 网友问：那如果原本的曲线就很好呢？本来横轴就大于纵轴，这样一搞，不久横轴变差了？
  - 网友答：这个意思就是尽可能的平均，大的让它小一点，小的让它大一点，不过这个是基于错误的方向比正确率的方向大这个假设上成立的
  - 网友答：这操作就像把dw和db归一化了，快的放慢，慢的加快。
  - 网友：不管是w还是b都是将较大变化的维度缩小，将较小变化的维度变大
...
- 网友问：那如果Sdw大于一，除以一个大于一的开根号，相当于学习率减小了呀 不是慢了吗？
  - 网友答：都可以，那个摆动大就消除哪个吧。
...
我们讲过了**Momentum，我们讲了RMSprop，如果二者结合起来，你会得到一个更好的优化算法**，在下个视频中我们再好好讲一讲为什么。
