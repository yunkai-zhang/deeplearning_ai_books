...
我知道在机器学习中，有时候我们很鄙视手工操作，或者使用了太多人为数值。但如果你要搭建应用系统，那这个简单的人工统计步骤，错误分析，可以节省大量时间，可以迅速决定什么是最重要的，或者最有希望的方向。实际上，如果你观察100个错误标记的开发集样本，也许只需要5到10分钟的时间，亲自看看这100个样本，并亲自统计一下有多少是狗。根据结果，看看有没有占到5%、50%或者其他东西。这个在5到10分钟之内就能给你估计这个方向有多少价值，并且**可以帮助你做出更好的决定，是不是把未来几个月的时间投入到解决错误标记的狗图这个问题**。
...
- 网友：他其实影响的不是模型的性能，而是对模型的评价指标，无法得到客观的对模型的评价指标。
...
- 我：把关心的数据集平均分散到train dev test是不合适的，因为目标是手机上传的图片，但是作为靶子的dev集合中手机上传的图片的比例太小了；相当于按照A靶子训练，然后射击B靶子。
...
- 我：这么看来，dev和test总是要使用目标数据集，训练集可以不用，因为可以从训练集训练的若干模型中，选出适合dev集的模型。
  - 网友复议：不论数据来自哪里，target都必须设置为你的实际应用场景。
- 网友：本来就是讲直觉 经验就很重要 习惯就好 并不突兀。
...
- 网友：原来这里的training-dev set是不参加训练的，所以就相当于普通的和trainset相同分布的devset，所以就是为了测试过拟合问题。
  - 我：如果train上的error和traindevset上的error类似，但是train上的error和devset的error相差较大；那说明模型没有出现过拟合，相反，模型表现得很好，只是因为trainset和devset没有同分布，导致的两个set上的error不同。
- 网友问：训练集和训练-开发集是同一分布，为什么错误率相差会很大？
  - 网友答：控制变量法，如果新的tra_dev还是在10左右就是过拟合，如果变为1了 就是因为之前的来自不同分布
...
- 我：上图讲述了通过各个数据集的error，看出究竟是那块的问题比较大。比如testerror是12%，那么看从human到testset之间过渡的error是主要在哪突然变大的
  - 网友：errorrate在大多数情况下，就像上图一样递增：humanhead<trainset<traindev<dev<test。其中一般来说traindevset是比devset的错误要大的，因为traindevset和trainset同分布，但是devset和trainset可能不是同分布；不过这个一般也有例外，下面会讲。
...
**这里还有个例子，其中数字并没有一直变大（我：例外在这）**，也许人类的表现是4%，训练错误率是7%，训练-开发错误率是10%。但我们看看开发集，你发现，很意外，算法在开发集上做的更好，也许是6%。所以如果你见到这种现象，比如说在处理语音识别任务时发现这样，其中训练数据其实比你的开发集和测试集难识别得多。所以这两个（7%，10%）是从训练集分布评估的，而这两个（6%，6%）是从开发测试集分布评估的。**所以有时候如果你的开发测试集分布比你应用实际处理的数据要容易得多，那么这些错误率可能真的会下降**。所以如果你看到这样的有趣的事情，可能需要比这个分析更普适的分析，我在下一张幻灯片里快速解释一下。
...
- 网友：这不叫噪音了吧
  - 我：1h的噪音回放10000次，导致噪音始终是这1h的噪音；但是现实来说，噪音可能是每时每刻都是不同的；所以重复的噪音可能会造成模型的噪音的过拟合。
...
我们谈到其中一种办法是人工数据合成，人工数据合成确实有效。在语音识别中。我已经看到人工数据合成显著提升了已经非常好的语音识别系统的表现，所以这是可行的。但当你使用**人工数据合成时，一定要谨慎，要记住你有可能从所有可能性的空间只选了很小一部分去模拟数据**。
...
- 网友：是的，那个大的网络叫老师网络，小的网络叫学生网络。学生网络可以通过老师网络进行学习，即使学生网络并没有训练但依旧可以达到很高的准确率。
...
- 网友：预训练+微调，[概念讲解](http://t.csdn.cn/d3xi1)
...
- 网友：跟多分类好像啊。
  - 恩达和网友反驳：多分类输出的y只有一个为一，就好像不能即是猫又是狗，但是这个多任务可以同时存在
...
- 网友：我觉得这样是和我们人更像了，我们看东西也是综合的
  - 我：红绿灯一般都和十字路口相伴出现，所以单独识别红绿灯or十字路口，不如用一个模型同时尝试识别图片里有没有红绿灯or十字路口。
...
- 网友：就类似于叠加一样，你学其他的东西总会有和你最后要学的东西有交集，那么就打好了一定的基础。
  - 网友：就是互相有联系，前面的low level的训练都是一样的。
...
- 网友问：为什么又拆开训练了（上图的第三点）？
  - 网友答：这里是在说多任务学习的替代方法，因为同时训练所有的任务可能会导致模型过大。
...
- 网友问：现在不是又开始强调可解释性了吗
  - 网友答：强调可解释性是根据业务需求来的吧？总不能跟客户说，因为我们也不理解原理的模型，把你干掉了
...
- 网友：所以火车站的要放上身份证，才能比对，要不然全中国几亿人，匹配到什么时候去。身份证相当于只需要匹配两张照片，不需要匹配当前人脸和14亿张照片。
...
- 网友：其实也是基于人类经验。我们识别一个人，通常也是先找到并聚焦人脸，然后依据记忆去判断人的身份。
...
- 网友：深度学习就是看数据呗，有怎样的数据就能训练出怎样的效果。
...
- 网友：端对端不就成了极大量数据下的不那么暴力的搜索乐么
  - 我：我也这么感觉，有点像redis中的kv键值对了吧？
