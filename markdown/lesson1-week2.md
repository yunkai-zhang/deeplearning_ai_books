...
- X矩阵代表了训练集，默认情况下是单条数据是一列，这会让构建神经网络更简单；偶尔情况下，X会表示成一行代表一个样本数据，但是这不利于构建nn（所以上图被叉掉了）
- Y表示训练集对应的结果，x1对应y1。
- 网友：如果Y是放在X的最后一列输入，也就是列向量，那确实是（m, ），但是这里老师的意思是单独输入的行向量那就是（1，m）
...
- 网友：深度学习激活函数用relu多；但是这里只是简单的逻辑回归，一般不会出现梯度消失，所以用logistic函数就足够了。
- 我：sigmoid函数把wx+b的输出转化成概率形式，来表示hat{y}，即表示y为某指定值的概率（本例的指定值为y==1==是猫）

![](../images/f5049dc7ce815b495fbbdf71f23fc66c.png)

- 我：可以看到theta0当做了b，theta1~thetanx当做了w；由于x0==1，所以b乘x0==1；所以thetaTX得到的结果和wt+b结果一样。
...
- 我：hat{y}==1==y表示，预测当前样本为 猫 的概率为 100%==1，这样也就解释了 概率和数值接近 的原因。
...
- 网友：零基础的建议去看 《深度学习的数学》，看完会有帮助理解这些。

我们在逻辑回归中用到的损失函数是：$L\left( \hat{y},y \right)=-y\log(\hat{y})-(1-y)\log (1-\hat{y})$

- 网友：这个公式可以用最大似然估计或者交叉熵推导出来。
- 网友：log应该写成ln，因为这里是e为底的。
  - 网友：底数不重要。
...
**损失函数（Loss）是在单个训练样本中定义的，它衡量的是算法在单个训练样本中表现如何，为了衡量算法在全部训练样本上的表现如何，我们需要定义一个算法的代价函数（Cost）**，算法的代价函数是对$m$个样本的损失函数求和然后除以$m$:
$J\left( w,b \right)=\frac{1}{m}\sum\limits_{i=1}^{m}{L\left( {{{\hat{y}}}^{(i)}},{{y}^{(i)}} \right)}=\frac{1}{m}\sum\limits_{i=1}^{m}{\left( -{{y}^{(i)}}\log {{{\hat{y}}}^{(i)}}-(1-{{y}^{(i)}})\log (1-{{{\hat{y}}}^{(i)}}) \right)}$
损失函数只适用于像这样的单个训练样本，而代价函数是参数的总代价，所以在训练逻辑回归模型时候，我们需要找到合适的$w$和$b$，来让代价函数 $J$ 的总代价降到最低。
根据我们对逻辑回归算法的推导及对单个样本的损失函数的推导和针对算法所选用参数的总代价函数的推导，结果表明**逻辑回归可以看做是一个非常小的神经网络**，在下一个视频中，我们会看到神经网络会做什么。
...
- 网友：国内外教材对于 凹凸函数 的定义是反的。凸函数就是向下凹陷的。
- 网友：这里的凸是在 优化原理 里面的凸，只有凸和非凸，不是凹和凸。
...
可以用如图那个小红点来初始化参数$w$和$b$，也可以采用随机初始化的方法（但人们一般不这么做），对于逻辑回归几乎所有的初始化方法都有效，因为函数是凸函数，无论在哪里初始化，应该达到同一点或大致相同的点。
...
- 网友问：这里应该是求偏导吧？
  - 网友答：因为恩达忽略了另一个参数，所以不用偏导。
    - 网友反驳：这就是求偏导，因为这里j有两个参数 w b，所以需要分别对他们求偏导。
  - 恩达：求导数或偏导时，当分子中有两个及以上的符号，为了明确语义，应该用 花体的d 表示偏导；如果分子中只有一个符号，那么就该用 d 表示导数。这是微积分中的一个奇特的符号规则，恩达认为它让事情变得更复杂了。
    - 网友反驳：因为d要表示全微分，所以必须要和偏导数区分开。


### 2.5 导数（Derivatives）

这个视频我主要是想帮你获得对微积分和导数直观的理解。或许你认为自从大学毕以后你再也没有接触微积分。这取决于你什么时候毕业，也许有一段时间了，如果你顾虑这点，请不要担心。**为了高效应用神经网络和深度学习，你并不需要非常深入理解微积分**。因此如果你观看这个视频或者以后的视频时心想：“哇哦，这些知识、这些运算对我来说很复杂。”我给你的建议是：坚持学习视频，最好下课后做作业，成功的完成**编程作业**，然后你就可以使用深度学习了。在第四周之后的学习中，你会看到定义的很多种类的函数，通过微积分他们能够帮助你把所有的知识结合起来，其中有的叫做前向函数和反向函数，因此你不需要了解所有你使用的那些微积分中的函数。所以你不用担心他们，除此之外在对深度学习的尝试中，这周我们要进一步深入了解微积分的细节。所有你只需要直观地认识微积分，用来构建和成功的应用这些算法。最后，如果你是精通微积分的那一小部分人群，你对微积分非常熟悉，你可以跳过这部分视频。其他同学让我们开始深入学习导数。
...
- 我理解：前向过程 输出结果；反向传输 用于计算导数，从而修正w和b
...
**所以这是一个计算流程图，就是正向或者说从左到右的计算来计算成本函数J（J是你可能需要优化的函数），然后反向从右到左计算导数**。如果你不熟悉微积分或链式法则，我知道这里有些细节讲的很快，但如果你没有跟上所有细节，也不用怕。在下一个视频中，我会再过一遍。在逻辑回归的背景下过一遍，并给你介绍需要做什么才能编写代码，实现逻辑回归模型中的导数计算。
...
- 网友：这是值，不是矩阵；这将向量乘法展开了；这里计算的是单个样本的预测值。
...
- 我：图中带引号的“da”，表示在代码中应该用这个变量来表示 该求导得到的值。
- 我：dL时，带不带 a y 都可以。
- 我：dl/dw1=x1*dl/dz，所以dz/dw1=x1,这符合求偏导数的规范。
- 网友：下边那一行dz老师没有加“”容易混淆。这里的dz是指python代码中是dz，在数学上相当于dl/dz。

- 我：求到`dz=a-y`的这一步非常精妙；这样就可以利用“dw1”=x1*dz得到针对当前样本x^1的输入，参数w1该怎么变化才能让Loss减少。
...
![image-20220822114748040](lesson1-week2.assets/image-20220822114748040.png)

- 我：上图的最后一行公式，表示的是怎么根据m个训练样本 结合 损失函数L 来计算参数w1的优化方向。
...
- 网友问：为什么要用J +=呀 求各位大佬解释？
  - 网友答：J+那个部分是因为J是m个单个训练样本的的和的平均数，这个J+=是求和的意思从1一直加到m
  - 网友：j+=是第一步是先进行foward propation，算出J
- 网友：上图是python的伪代码，这里是把每一个样本的dw都加在一起求和，最后会算平均的。求平均是让每一个特征都能影响权重
- 恩达：左侧是已经计算了损失函数J 对各个参数w1 w2和b的导数。
- 网友：这个实质应该是把大的数据集划分为m为一小组的若干小批量样本，用这一小批量样本的损失函数均值来代替单个样本的损失函数。
- 网友：w1和w2只更新了一次所以只用了一次梯度下降，遍历完所有样本只更新一次w1和w2。
- 我：α是学习速率，可以自己调节这个值的大小，来设置梯度的下降速度。
- 网友：因为一共有m个训练样本呀 前面有m个损失函数的累加 所以要除以m
- 网友问：是更新一次初始值就要求导一次再减是吧？这里是指不断前向后向迭代，cost func最终会收敛到最低点吗？
...
- 网友：如果你的w有100个，你总不能写w1....w100吧？这个时候就要第二个for进行遍历。
...
- 我：$z={{w}^{T}}x+b$，这样可以算到一个具体的值z。w因为转置过，所以和x相乘后是一个值。
...
- 网友问：我运行出来怎么数值都有点差异，小数点最后几位
  - 网友答：因为计算机是二进制，不是所有数都能表示成二进制整数，计算机精确到小数点后八位，计算后再变为十进制数自然不一样。
...
- 高赞网友：for loop 中代码没有错，遍历了所有的u的分量。错的是上面ui的表示，多了对i求和
  - 网友反驳：老师说了是矩阵乘以向量 没多。最后求出来的u是一个列向量，ui是列向量的i个元素
...
事实上，**numpy**库有很多向量函数。比如 `u=np.log`是计算对数函数($log$)、 `np.abs()` 计算数据的绝对值、`np.maximum(v, 0)` 按元素计算$v$中每个元素和和0相比的最大值，`v**2` 代表获得元素 $v$ 每个值的平方、 `1/v` 获取 $v$ 中每个元素的倒数等等。**所以当你想写循环时候，检查numpy是否存在类似的内置函数，从而避免使用循环(loop)方式。**
...
- 我：针对所有样本的遍历是一个外部循环；针对每个w的处理是一个内部循环。
...
![image-20220826152800338](lesson1-week2.assets/image-20220826152800338.png)

- 网友：x下标代表位置参数w机前面的系数，上标代表第几个样本
- 网友：为什么是dw是X和dx转置的平均？
  - 我：针对一个batch来更新一次w，所以dw应该是一个batch所包含的样本所计算得的dw的平均值。
  - 网友：所有样本 想要下降所以样本得cost 所以算均值。
    - 我：（之前推导反向传播的公式时，相当于把batch设置为1，即针对每个样本都更新一次w）
...
![image-20220826153620998](lesson1-week2.assets/image-20220826153620998.png)

- `:=`是赋值符号
- 恩达：如果想进行1000次的梯度下降的话，我们就还是需要一个for循环来计算次数；这个for是没办法省略或替换的。
  - 网友问：循环1000次迭代有什么用，不已经求出最优解了吗？
    - 网友答：求导之后改变了w，从而改变了Z和A，然后进行新一轮的求导；梯度下降是每次下降一点。
    - 网友答：不是最优的 有步率得 每次下降一些；梯度下降很多次后才能得到最优的w。
  - 网友：个人理解：在梯度下降过程中，J的值是不重要的，重要的J值对应点的梯度。通过梯度去决定参数的下降方向。
    - 网友：J已经体现在了dz=a-y中了，dz是由J反向推算出来的
...
- 网友：这里可以不用reshape，因为cal本身就是(1,4)的维度的二维矩阵。
  - 网友反驳： 不不不，cal是一维向量，用reshape才能变成二维矩阵，才能相除。
    - 网友反驳 ：本来是一维的，可以输出下cal.shape就能看出来，不过这里不reshape()也行
  - 网友：矩阵和数组最好还是区分一下，矩阵shape是(1,4)数组是(4,) 不用reshape的原因是numpy自己的程序健壮性好
- 恩达：编写py代码时，如果不确定矩阵的尺寸，常常使用reshape命令来确认。
...
就我们刚才看到的，再进一步说明。首先我们刚刚运行的命令是这个 $(a=np.random.randn(5))$，它生成了一个数据结构$a$，其中$a.shape$是$(5,)$。这被称作 $a$ 的一维数组（**秩为1的数组 （rank 1 array）**），同时这也是一个非常有趣的数据结构。它不像行向量和列向量那样表现的很一致，这使得它带来一些不直观的影响。所以我建议，当你在编程练习或者在执行逻辑回归和神经网络时，你不需要使用这些一维数组。
...
- 网友：y^的概率意义是认为给予的。
- 恩达：y帽 表示给定训练样本x条件下y等于1的概率。
  - 恩达：需要指出的是，我们目前讨论的是2分类问题，所以y的取值只能是0或1.
...
- 网友：面试题目：为什么用log
  - 网友答：用log的原因：求导比较方便。
  - 网友答：这里只是用对数的一种解释，还有其他的损失函数解释可以看机器学习的视频里
- 网友问：为什么对数函数单调递增就用对数函数，很多函数都是单调递增啊？
  - 网友答：转换成对数函数是因为求导简单，兄嘚；
  - 网友答：使用log函数的原因是它在0，1之间变化大，
  - 网友答：两边取对数，就把指数函数转化为乘积的形式
  - 网友答：因为用的其实是似然函数，没学过的可以去看看mle
