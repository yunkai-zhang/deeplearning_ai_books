...
- 我：语言模型能做两个事情主要：
  - 估计给定句子的可能性
  - 随机给一个输出输入，进而随机生成一个新的句子

![](../images/a8b8c64483ee84d57135829ab025da53.png)

- 我：用encodingmodule代替语言模型中的0向量的a<0>，就得到了machine translation。
...
- 我：因为going的频率出现打，所以会出现单词局部最优的第二句，而不是更简洁的全局最优的第一句。
...
**贪婪算法只会挑出最可能的那一个单词，然后继续。而集束搜索则会考虑多个选择，集束搜索算法会有一个参数B，叫做集束宽（beam width）**。在这个例子中我把这个集束宽设成3，这样就意味着集束搜索不会只考虑一个可能结果，而是一次会考虑3个，比如对第一个单词有不同选择的可能性，最后找到**in**、**jane**、**september**，是英语输出的第一个单词的最可能的三个选项，然后集束搜索算法会把结果存到计算机内存里以便后面尝试用这三个词。如果集束宽设的不一样，如果集束宽这个参数是10的话，那么我们跟踪的不仅仅3个，而是10个第一个单词的最可能的选择。所以要明白，为了执行集束搜索的第一步，你需要输入法语句子到编码网络，然后会解码这个网络，这个**softmax**层（上图编号3所示）会输出10,000个概率值，得到这10,000个输出的概率值，取前三个存起来。
...
- 我：这里应该不是 in jane september 每个都选一个，而是30000个里，综合概率选三个出来，这三个可能都来自in里。
...
- 我：就像根据第一个单词推第二个，这里根据前两个单词推出第三个！
...
- 网友：概率越大取log也越大越接近0，累加最大时也是累乘最大
- 网友：这里的log应该就是ln；ln单增函数，所以可以不取负。
- 我：x越接近0，那么lnX就越小（负数地越厉害）；
...
- 我：在避免模型倾向于选择短句子，可以让结果除以句子长度；同时还可以给句子长度设置一个指数，来调节句子长短对模型的影响程度。
...
- 我：上图说明，从RNN和Bean search中找谁是错误的主要因素。
...
- 网友：我是学图片识别的把注意力模型前面的自言语言处理与定向搜索跳过了，应该没问题吧
