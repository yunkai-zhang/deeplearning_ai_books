...
- 我：因为是全连接层，所以每个像素都要送给1000个隐藏单元，一共有300万个像素，所以有1000*300万个权重。
...
在之前的视频中，我说过**神经网络的前几层是如何检测边缘的，然后，后面的层有可能检测到物体的部分区域，更靠后的一些层可能检测到完整的物体**，这个例子中就是人脸。在这个视频中，你会看到如何在一张图片中进行边缘检测。

![](../images/47c14f666d56e509a6863e826502bda2.png)

- 网友：原来是分层次检测的，这样前面的迁移学习就好理解了
  - 网友：和迁移学习有关在于：底层特征都是相似的，可以共用，高层则是将这些特征进行不同的组合
...
看一个例子，这是一个6×6的灰度图像。因为是灰度图像，所以它是6×6×1的矩阵，而不是6×6×3的，因为没有**RGB**三通道。为了检测图像中的垂直边缘，你可以构造一个**3×3矩阵。在共用习惯中，在卷积神经网络的术语中，它被称为过滤器**。我要构造一个3×3的过滤器，像这样$\begin{bmatrix}1 & 0 & -1\\ 1 & 0 & -1\\ 1 & 0 & -1\end{bmatrix}$。在论文它有时候会被称为核，而不是过滤器，但在这个视频中，我将使用过滤器这个术语。对这个6×6的图像进行卷积运算，卷积运算用“$*$”来表示，用3×3的过滤器对其进行卷积。

- 网友：这里的过滤器 就是 cnn里的 卷积核
- 网友：这和信号处理中的卷积有点不同，感觉更像相关运算
...
- 网友问：卷积计算要翻转180°，那么这里的卷积和为什么不翻转？
  - 网友答：前面说得不太好，实际上因为卷积核在学习过程中会不断迭代，翻转毫无意义，反而增加开销，就好像人眼看东西都是倒立的，但大脑会自动翻转，没必要给正常人前面加个翻转镜片
...
- 我：右边这个矩阵就可以看出横向的边缘，从正负还能看出是从明往暗处还是从暗往明。
...
但计算机视觉的研究者们也会经常使用其他的数字组合，比如这种：$\begin{bmatrix} 3& 0 & - 3 \\ 10 & 0 & - 10 \\ 3 & 0 & - 3 \\\end{bmatrix}$，这叫做**Scharr过滤器，它有着和之前完全不同的特性，实际上也是一种垂直边缘检测，如果你将其翻转90度，你就能得到对应水平边缘检测**。
...
- 网友：毕竟是表示边缘  总要分  两边和中间    其他的应该也可以   只是这个个细致  最后出来的边缘也会更准确
...
**所以这种将这9个数字当成参数的思想，已经成为计算机视觉中最为有效的思想之一**。在接下来的课程中，也就是下个星期，我们将详细去探讨如何**使用反向传播去让神经网络学习这9个数字**。但在此之前，我们需要先讨论一些其它细节，比如一些基础的卷积运算的变量。在下面两节视频中，我将与你们讨论如何去使用**padding**，以及卷积各种不同的发展，这两节内容将会是卷积神经网络中卷积模块的重要组成部分，所以我们下节视频再见。
...
第二个缺点时，如果你注意角落边缘的像素，这个像素点（绿色阴影标记）只被一个输出所**触碰或者使用**，因为它位于这个3×3的区域的一角。但如果是在中间的像素点，比如这个（红色方框标记），就会有许多3×3的区域与之重叠。**所以那些在角落或者边缘区域的像素点在输出中采用较少，意味着你丢掉了图像边缘位置的许多信息**。
...
习惯上，**计算机视觉中，$f$（卷积核的边长）通常是奇数**，甚至可能都是这样。你很少看到一个偶数的过滤器在计算机视觉里使用，我认为有两个原因。

**其中一个可能是**，如果$f$是一个偶数，那么你只能使用一些不对称填充。只有$f$是奇数的情况下，**Same**卷积才会有自然的填充，我们可以以同样的数量填充四周，而不是左边填充多一点，右边填充少一点，这样不对称的填充。

**第二个原因是**当你有一个奇数维过滤器，比如3×3或者5×5的，它就有一个中心点。有时在计算机视觉里，如果有一个中心像素点会更方便，便于指出过滤器的位置。
...
- 网友：或者理解为沿Y=X翻转也可以
  - 网友反驳：这里应该是错了，直接按对角线镜像了，应该是依次水平和垂直镜像
...
- 我理解：虽然dl中的“卷积操作”没有做镜像，严格意义上只能叫 互相关，但是按照管理我们也称之为卷积了。
...
事实证明在信号处理中或某些数学分支中，**在卷积的定义包含翻转，使得卷积运算符拥有这个性质**，即$(A*B)*C=A*(B*C)$，这在数学中被称为**结合律**。这对于一些信号处理应用来说很好，但对于深度神经网络来说它真的不重要，因此省略了这个双重镜像操作，就简化了代码，并使神经网络也能正常工作。

- 网友：神经网络中卷积核中的格子都是参数，可以通过训练得出，所以没必要进行翻转。
...
给这些起个名字（原图像），这里的第一个6代表图像高度，第二个6代表宽度，这个3代表通道的数目。同样你的过滤器也有一个高，宽和通道数，并且**图像的通道数必须和过滤器的通道数匹配**，所以这两个数（紫色方框标记的两个数）必须相等。下个幻灯片里，我们就会知道这个卷积操作是如何进行的了，**这个的输出会是一个4×4的图像，注意是4×4×1，最后一个数不是3了**。
...
- 恩达：多维卷积核中R不全为0，GB对应全为0；这是检测了红色的垂直边缘。
  - 网友：但可以有多个卷积核啊，所以输出可以多通道的；一个卷积核是3维度的，有多个卷积核。
- 恩达：多维卷积核中RGB都不为0，那么就不在乎颜色，而是就检测垂直边缘。
...
- 网友：最后卷积的结果作为下一层卷积的开始啊 不用叠加，就像处理前面这个3通道的图片一样
- 网友：灰度图像就一个channel
...
- 我：这里把每个像素当成一个parameter；一个卷积核包含若干个参数；一个偏置用于一个卷积核；所以一个卷积核类似于之前的一个神经元
...
那么通道数量又是什么？这些数字从哪儿来的？我们来看一下。输出图像也具有深度，通过上一个示例，我们知道它等于该层中过滤器的数量，如果有2个过滤器，输出图像就是4×4×2，它是二维的，如果有10个过滤器，输出图像就是4×4×10。**输出图像中的通道数量就是神经网络中这一层所使用的过滤器的数量**。如何确定过滤器的大小呢？我们知道卷积一个6×6×3的图片需要一个3×3×3的过滤器，因此**过滤器中通道的数量必须与输入中通道的数量一致**。因此，输出通道数量就是输入通道数量，所以过滤器维度等于$f^{[l]} \times f^{[l]} \times n_{c}^{\left\lbrack l - 1 \right\rbrack}$。

![](../images/53d04d8ee616c7468e5b92da95c0e22b.png)

- 网友：懂了，原来上标[l]是指第几层
- 网友：过滤器的通道数等于上一层的通道数，而当前层的输出的通道数等于过滤器的个数 
- 我：这里的f指filtersize，也就是过滤器的长or高（默认长==高）。
...
卷积有很多种标记方法，这是我们最常用的卷积符号。大家在线搜索或查看开源代码时，**关于高度，宽度和通道的顺序并没有完全统一的标准**卷积，所以在查看**GitHub**上的源代码或阅读一些开源实现的时候，你会发现有些作者会采用把通道放在首位的编码标准，有时所有变量都采用这种标准。实际上在某些架构中，当检索这些图片时，会有一个变量或参数来标识计算通道数量和通道损失数量的先后顺序。只要保持一致，这两种卷积标准都可用。很遗憾，这只是一部分标记法，因为深度学习文献并未对标记达成一致，但**课上我会采用这种卷积标识法，按高度，宽度和通道损失数量的顺序依次计算**。
...
- 网友：我理解是卷积后图像中较大的值就代表着提取的特征（例如边缘检测），那么取各部分的最大值就相当于保留了提取的特征。
- 网友：也就是说大佬们用它主要因为池化在实践中被证明有用，而不是因为从原理上证明它有用

必须承认，**人们使用最大池化的主要原因是此方法在很多实验中效果都很好**。尽管刚刚描述的直观理解经常被引用，不知大家是否完全理解它的真正原因，不知大家是否理解最大池化效率很高的真正原因。

其中一个**有意思的特点就是，它有一组超参数，但并没有参数需要学习**。实际上，梯度下降没有什么可学的，一旦确定了$f$和$s$，它就是一个固定运算，梯度下降无需改变任何值。
...
- 网友：就是400个特征，输入到有120个神经元的隐层。隐藏单元个数自己可以根据实际情况调整。
- 网友：你就理解为，本来就是很简单的神经网络，但是在训练前，用了卷积核和最大池化进行预处理。提取新的特征
...
- 网友：原来这就是全连接啊
- 网友：为啥要加一个全连接层？
  - 网友答：前面的layer1、layer2的目的感觉是减少数据量 提取特征，layer3开始熟悉的网络迭代
...
- 网友：我理解的是卷积和池化仅仅是将输入进行个格式转化，反向只到卷积的最后一步就行了，卷积和FC并不是交替进行
...
- 网友问：CONV1参数为什么是208
  - 因为每个核有25个参数，共8个核就是200个，加上8个偏差。

**有几点要注意**，**第一**，池化层和最大池化层没有参数；**第二**卷积层的参数相对较少，前面课上我们提到过，其实许多参数都存在于神经网络的全连接层。观察可发现，随着神经网络的加深，激活值尺寸会逐渐变小，**如果激活值尺寸下降太快，也会影响神经网络性能**。示例中，激活值尺寸在第一层为6000，然后减少到1600，慢慢减少到84，最后输出**softmax**结果。我们发现，许多卷积网络都具有这些属性，模式上也相似。

- 网友：可能不是一样的参数,吴老师之前说是可以提取红色通道的边缘和绿色通道的边缘,所以这完全可以用两个滤波器实现,所以对于绿色通道的边缘,红色和蓝色就是0,应该不用计算,所以没有乘通道数
...
- 网友：理解：卷积时提取特征，相当于扣掉了不重要的背景，偏差参数相当于调低或调高亮度（特征敏感度）
...
- 网友：关于稀疏连接，1.就是使用相同的卷积核，2.就是每个元素只与那九个元素相关，减少了很多没有关系的映射。
  - 我：相当于相邻两个卷积层的输出结果的映射更稀疏了；是卷积核让两个卷积层的输出结果映射起来。
