...
- 我：所以train和dev可以不是同一分布；但是dev和test得确保是同一分布。
...
- 网友：方差（variance）：训练集和真实结果的差别；偏差（bias）：训练集和验证集之间的差别；
- 我：highbias就是欠拟合（train和dev的表现都不好），highvariance就是过拟合（train表现很好，但是dev表现不佳 ）
...
- 我：本图就是神奇的 highbias+highvariance
...
- 网友：其实就是两者此消彼长，w太太大右边的很大左边很小导致损失函数仍然很大，反之损失函数也是很大，所以应该均衡两者
- 网友：我的理解，过拟合是每个数据太过接近，导致成本函数很小，加上这个正则化会增加成本函数，改善过拟合
...
- 网友问：为什么要引入范数？
  - 网友答：因为范数可以约束W的value，如果很多w的value都为0，效果就和dropout一样
  - 网友答：这里w表示原始未转置矩阵，推公式时这样写比较规范，编程时预先转置方便计算
...
- 网友：正则化就是为了降低模型的复杂度进而避免过拟合。
- 网友：一般过拟合高方差的其中一个原因是：模型复杂度过高，而加入正则，可以降低某些特征的权重，达到模型复杂度降低的效果。
  - 网友：一视同仁的，不是盯着某个特点的权重去减。
...
- 网友：直观理解，W权重小了相当于减小各方向梯度的影响，也就是减小对数据集的依赖性，所以会导致欠拟合
- 网友：我的理解：过拟合就是因为学习的权重太多所导致的，所以预防过拟合就是尽量减少学习的权重数量。因为加上正则项之后，就会让某些权重约为0，这样就减少权重数量了
- 网友：其实就是多目标优化问题，模型精度和模型大小是两个冲突的目标
...
现在你应该摒弃这个直觉，**如果正则化参数λ很大，激活函数的参数会相对较小，因为代价函数中的参数变大了**

- 网友问：因为代价函数的参数变大了,所以要摒弃拉姆他变大,参数变小的想法,这什么逻辑啊
  - 网友答：相当于后面L2正则化的权重更大了，为了使总误差更小，需要W更小
  - 网友：其实就是惩罚函数
...
- 网友：非线性环节会让模型变复杂，如果落在线性区，模型会简化，就不容易过拟合了
- 网友：relu在x负半轴不是线性的，relu对一些小于0的权重置零，即消除了一些神经元的作用；所以relu也起到了避免过拟合的作用。
...
- 网友问：这样的调试不就偏离了深度学习使用更多隐藏层的初衷么

  - 网友答：他就是原来的方法 过度拟合了许多的特殊情况 所以用这个方法让他回退一点

  - 网友答：某些情况下如果模型复杂一点收益大幅增加，正则化仍然会找出它。总之，一切都扔给学习过程去迭代搜索。
...
- 网友：这个实际上是集成方法，相当于构造了很多个子网络
- 网友问：那为啥不直接减少神经元个数？
- 网友：有随机森林内味儿了

![](../images/70b248490e496fed9b8d1d616e4d8303.png)

**如何实施dropout呢？方法有几种，接下来我要讲的是最常用的方法，即inverted dropout（反向随机失活）**，出于完整性考虑，我们用一个三层（$l=3$）网络来举例说明。编码中会有很多涉及到3的地方。我只举例说明如何在某一层中实施**dropout**。

首先要定义向量$d$，$d^{[3]}$表示网络第三层的**dropout**向量：

`d3 = np.random.rand(a3.shape[0],a3.shape[1])`

- 网友问：d3表示的是第三层的dropout参数，那它不应该是个向量吗？因为一个1或0表示一个unit是否失活，某个unit失活实际上就是让a3中的一行清零？
...
- 高赞网友：
  1. 就是先生成跟a3隐藏层相同shape的随机数d3；
  2. 然后依据 “小于0.8等于1，大于0.8等于0”的原则 把d3矩阵转化成一个0，1矩阵；
  3. 把d3和a3相乘，从而把a3中的系数剔除20%（就是遇到d3矩阵的0元素的那些a3的系数归零）
- 网友：d3是一个只有0和1的矩阵，然后和a3相乘就会剔除a3中20%的元素
...
- 网友：注意期望公式E(X)=0.2*0+0.8*x，E(X)/0.8=x期望就不变了。
...
- 高赞网友：
  1. 用随机数产生01向量，每个向量表示当前层的神经元是否保留，保留的神经元概率为keepprob
  2. 按原先backprob的方法计算A矩阵，乘以那个向量就是dropout后的结果
  3. 因为删掉了很多节点，为保证dropout后的结果保持原先的期望，因此要除以keepprob
- 网友：除以0.8的是a，而不是w。
...
- 网友：就是说在测试阶段不用dropout
- 网友问：说啥呢在这，到底测试时候用不用dropout啊？
  - 网友答：不需要用，因为你在训练模型的时候就考虑了期望的问题
...
注意**keep-prob**的值是1，意味着保留所有单元，并且不在这一层使用**dropout**，对于有可能出现过拟合，且含有诸多参数的层，我们可以把**keep-prob**设置成比较小的值，以便应用更强大的**dropout**，有点像在处理$L2$正则化的正则化参数$\lambda$，我们尝试对某些层施行更多正则化，**从技术上讲，我们也可以对输入层应用dropout，我们有机会删除一个或多个输入特征，虽然现实中我们通常不这么做，keep-prob的值为1，是非常常用的输入值**，也可以用更大的值，或许是0.9。但是消除一半的输入特征是不太可能的，如果我们遵守这个准则，**keep-prob**会接近于1，即使你对输入层应用**dropout**。

- 网友：如果这个参数指的是保留的概率，那是不是有概率全部删除？那如果全部删除了要怎么传递到下一层？
...
- 网友问：为什么输入像素太多，就没有足够数据了？有啥关系？？
  - 网友答：可以理解为存储空间一定，一个样本的特征量太多的话，就放不了太多样本，数据量就小了吗
  - 网友答：像素多所以导致样本少，没法通过增加输入样本的方法提高过拟合，只能通过正则化

![](../images/13987e28fa537a110c6b123fc4455f7c.png)

**dropout**一大缺点就是代价函数$J$不再被明确定义，每次迭代，都会随机移除一些节点，如果再三检查梯度下降的性能，实际上是很难进行复查的。定义明确的代价函数$J$每次迭代后都会下降，因为我们所优化的代价函数$J$实际上并没有明确定义，或者说在某种程度上很难计算，所以我们失去了调试工具来绘制这样的图片（j函数单调递减的图片）。我通常会关闭**dropout**函数，将**keep-prob**的值设为1，运行代码，确保J函数单调递减。然后打开**dropout**函数，希望在**dropout**过程中，代码并未引入**bug**。我觉得你也可以尝试其它方法，虽然我们并没有关于这些方法性能的数据统计，但你可以把它们与**dropout**方法一起使用。

- 网友：这里我想了很久，J不被明确定义是因为J是W1 W2...的函数，因为缺失了这样一个变量，那么算出来的J自然是没有变化意义的。
- 网友：所以关闭dropout关闭的方式就是把那个keepalive参数直接设置为1。
...
- 网友：w=w-dw 在此处dw是负值 所以w会增大。

术语**early stopping**代表提早停止训练神经网络，训练神经网络时，**我有时会用到early stopping，但是它也有一个缺点，我们来了解一下**。
...
但对我来说**early stopping的主要缺点就是你不能独立地处理这两个问题，因为提早停止梯度下降，也就是停止了优化代价函数$J$**，因为现在你不再尝试降低代价函数$J$，所以代价函数$J$的值可能不够小，同时你又希望不出现过拟合，你没有采取不同的方式来解决这两个问题，而是用一种方法同时解决两个问题，这样做的结果是我要考虑的东西变得更复杂。
...
- 网友：离散化，归一化，标准化，都可以
  - 网友补充：标准化好，因为中国的数学课本都是用标准化。
  - 网友：其实 从名词上讲应该只有归一化，而 标准化只是归一化的一种方法
...
然而如果**你归一化特征，代价函数平均起来看更对称**，如果你在上图这样的代价函数上运行梯度下降法，你必须使用一个非常小的学习率。因为如果是在这个位置，梯度下降法可能需要多次迭代过程，直到最后找到最小值。但**如果函数是一个更圆的球形轮廓，那么不论从哪个位置开始，梯度下降法都能够更直接地找到最小值，你可以在梯度下降法中使用较大步长，而不需要像在左图中那样反复执行**。

当然，实际上$w$是一个高维向量，因此用二维绘制$w$并不能正确地传达并直观理解，但总地直观理解是**代价函数会更圆一些，而且更容易优化**，前提是特征都在相似范围内，而不是从1到1000，0到1的范围，而是在-1到1范围内或相似偏差，这使得代价函数$J$优化起来更简单快速。
...
- 我：之所以是指数增长，是因为每一层都要乘以1.5，这样乘以很多层之后就是1.5的指数次。
...
- 网友问：这种梯度爆炸情况只有在所有w都大于1时才会出现吧，实际中这种情况发生的多吗？
...
- 网友问：所以就是应该是标准化一下吧？之前归一化的方差我纠结了好久
  - 网友答：方差描述的是数据波动性，设置一组权重的方差变的小，是为了让xielu这一组权重集中化，不至于太分散，产生梯度爆炸或者消失
- 我：这里的random函数是高斯随机函数，上图下部的紫色方框是用于更好地初始化参数。
...
- 网友问：为什么？大的更接近
  - 网友答：类似中值定理吧。这是中值定理 不是导数定义。如果用右边，斜率偏大，左边又偏小，所以用中间来中和一下吧。
...
- 网友：之前我们推导出了梯度的公式，我们每一次用梯度公式算出来一个梯度然后进行梯度下降，我们设置这个debug函数就是为了测试我们之前求的公式对不对。
- 网友：就用一个另一种近似的方式计算导数，看你算的两个接近不，差距比较大就是出问题了
...
第四点，梯度检验不能与**dropout**同时使用，因为每次迭代过程中，**dropout**会随机消除隐藏层单元的不同子集，难以计算**dropout**在梯度下降上的代价函数$J$。因此**dropout**可作为优化代价函数$J$的一种方法，但是代价函数J被定义为对所有指数极大的节点子集求和。而在任何迭代过程中，这些节点都有可能被消除，所以很难计算代价函数$J$。你只是对成本函数做抽样，用**dropout**，每次随机消除不同的子集，所以很难用梯度检验来双重检验**dropout**的计算，所以我一般不同时使用梯度检验和**dropout**。如果你想这样做，可以把**dropout**中的**keepprob**设置为1.0，然后打开**dropout**，并寄希望于**dropout**的实施是正确的，你还可以做点别的，比如修改节点丢失模式确定梯度检验是正确的。实际上，我一般不这么做，**我建议关闭dropout，用梯度检验进行双重检查，在没有dropout的情况下，你的算法至少是正确的；然后打开dropout**。

- 网友：就是说dropout会随机消除一些隐藏单元，每次都不一样，这样的化梯度检验就没有可靠性了
  - 网友补充：你用了dropout你算出来的梯度实际上跟对结果是有一定影响的，不可靠
